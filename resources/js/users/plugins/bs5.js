...
const bs5 = {
  Toast: class {
    ...
    constructor(param) {
      ...
      this.#delay         = this.#setOption(param.delay, 3000)
      ...
      if (el instanceof HTMLDivElement) {
        ...
        btns.forEach((btn) => {
          this.#btnClose && btn.removeAttribute('hidden') // show btn-close
          this.#btnCloseWhite && btn.classList.add('btn-close-white') // add btn-close-white
        })
        if (this.#header !== '') {
          el.querySelector('.toast-header').removeAttribute('hidden') // show header
          btns[1].remove() // remove btn-close from toast-body
        }
        this.element = el
      }
      ...
      this.element.addEventListener('show.bs.toast', () => {
        ...
        function myFunction() {
          if (that.element.offsetHeight > 0) { // make sure element has height
            clearInterval(timer)

            if (that.#animation) {
              const transition = parseFloat(getComputedStyle(that.element).transitionDuration) * 1000
              that.element.style.transition = `all ${transition * 4}ms cubic-bezier(0.16, 1, 0.3, 1), opacity ${transition}ms linear`
              that.element.style[that.#pos[1]] = 0 // slide in
            }
            that.#stack()
          }
        }
